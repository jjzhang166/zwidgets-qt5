...
class ExtendWidgetPrivate : public ZObjectPrivate
{
    explicit ExtendWidgetPrivate(ExtendWidget *qq):
        ZObjectPrivate(qq) {}
    ...
    Z_DECLARE_PUBLIC(ExtendWidget)
};

ExtendWidget::ExtendWidget(QWidget *w, QObject *parent):
    QObject(parent),
    ZObject(*new ExtendWidgetPrivate(this))
{
    if (w) {
        Q_D(ExtendWidget);
...
class AnchorsBasePrivate
{
    ...
    AnchorInfo *top = new AnchorInfo(q_func(), Qt::AnchorTop);
    AnchorInfo *bottom = new AnchorInfo(q_func(), Qt::AnchorBottom);
    AnchorInfo *left = new AnchorInfo(q_func(), Qt::AnchorLeft);
    AnchorInfo *right = new AnchorInfo(q_func(), Qt::AnchorRight);
    AnchorInfo *horizontalCenter = new AnchorInfo(q_func(), Qt::AnchorHorizontalCenter);
    AnchorInfo *verticalCenter = new AnchorInfo(q_func(), Qt::AnchorVerticalCenter);
    ExtendWidget *fill = new ExtendWidget(NULL, q_func());
    ExtendWidget *centerIn = new ExtendWidget(NULL, q_func());
    int margins = 0;
    int topMargin = 0;
    int bottomMargin = 0;
    int leftMargin = 0;
